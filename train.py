...
# Task config
tf.flags.DEFINE_string("task_dataset_info", "square_room", "Name of the room in which the experiment is performed.")
...
tf.flags.DEFINE_list("task_pc_scale", [0.01], "Place cell standard deviation parameter (meters).")
tf.flags.DEFINE_list("task_n_hdc", [12], "Number of target head direction cells.")
tf.flags.DEFINE_list("task_hdc_concentration", [20.0], "Head direction concentration parameter.")
tf.flags.DEFINE_integer("task_neurons_seed", 8341, "Seeds.")
tf.flags.DEFINE_string("task_targets_type", "softmax", "Type of target, soft or hard.")
tf.flags.DEFINE_string("task_lstm_init_type", "softmax", "Type of LSTM initialisation, soft or hard.")
tf.flags.DEFINE_bool("task_velocity_inputs", True, "Input velocity.")
tf.flags.DEFINE_list("task_velocity_noise", [0.0, 0.0, 0.0], "Add noise to velocity.")

# Model config
tf.flags.DEFINE_integer("model_nh_lstm", 128, "Number of hidden units in LSTM.")
tf.flags.DEFINE_integer("model_nh_bottleneck", 256, "Number of hidden units in linear bottleneck.")
tf.flags.DEFINE_list("model_dropout_rates", [0.5], "List of floats with dropout rates.")
tf.flags.DEFINE_float("model_weight_decay", 1e-5, "Weight decay regularisation")
tf.flags.DEFINE_bool("model_bottleneck_has_bias", False, "Whether to include a bias in linear bottleneck")
...
tf.flags.DEFINE_integer("training_steps_per_epoch", 1000, "Number of optimization steps per epoch.")
tf.flags.DEFINE_integer("training_minibatch_size", 10, "Size of the training minibatch.")
tf.flags.DEFINE_integer("training_evaluation_minibatch_size", 1000, "Size of the minibatch during evaluation.")
tf.flags.DEFINE_string("training_clipping_function", "utils.clip_all_gradients", "Function for gradient clipping.")
tf.flags.DEFINE_float("training_clipping", 1e-5, "The absolute value to clip by.")

tf.flags.DEFINE_string("training_optimizer_class", "tf.train.RMSPropOptimizer", "The optimizer used for training.")
...
# Store
tf.flags.DEFINE_string("saver_results_directory", None, "Path to directory for saving results.")
tf.flags.DEFINE_integer("saver_eval_time", 10, "Frequency at which results are saved.")
...
def train():
    ...
    target_ensembles_path = "./checkpoints/target_ensembles.npy"
    if os.path.isfile(target_ensembles_path):
        np_array = np.load(target_ensembles_path)
        place_cell_ensembles[0].means = np_array["place_mean"]
        place_cell_ensembles[0].variances = np_array["place_variance"]
        head_direction_ensembles[0].means = np_array["head_mean"]
        head_direction_ensembles[0].kappa = np_array["head_kappa"]
    else:
        np.savez_compressed(
            target_ensembles_path,
            place_mean=place_cell_ensembles[0].means,
            place_variance=place_cell_ensembles[0].variances,
            head_mean=head_direction_ensembles[0].means,
            head_kappa=head_direction_ensembles[0].kappa,
        )
    ...
    if FLAGS.task_velocity_inputs:
        ...
        vel_noise = (
            tf.distributions.Normal(0.0, 1.0).sample(sample_shape=ego_vel.get_shape()) * FLAGS.task_velocity_noise
        )
    ...
    with tf.train.SingularMonitoredSession() as sess:
        for epoch in range(FLAGS.training_epochs):
            ...
            tf.logging.info("Epoch %i, mean loss %.5f, std loss %.5f", epoch, np.mean(loss_acc), np.std(loss_acc))
            if epoch % FLAGS.saver_eval_time == 0:
                res = dict()
                for _ in list(range(FLAGS.training_evaluation_minibatch_size // FLAGS.training_minibatch_size)):
                    mb_res = sess.run(
                        {
                            "bottleneck": bottleneck,
                            "lstm": lstm_output,
                            "pos_xy": target_pos,
                            "place_cell": ensembles_logits[0],
                        }
                    )
                    res = utils.concat_dict(res, mb_res)

                # Store at the end of validation
                idx = np.argmax(res["place_cell"], axis=2)
                predict_position = place_cell_ensembles[0].means[idx]
                utils.plot_trajectory(res["pos_xy"], predict_position, epoch)
                filename = "rates_and_sac_latest_hd.pdf"
                (
                    grid_scores["btln_60"],
                    grid_scores["btln_90"],
                    grid_scores["btln_60_separation"],
                    grid_scores["btln_90_separation"],
                ) = utils.get_scores_and_plot(
                    latest_epoch_scorer,
                    res["pos_xy"],
                    res["bottleneck"],
                    FLAGS.saver_results_directory,
                    filename,
                    epoch,
                )
