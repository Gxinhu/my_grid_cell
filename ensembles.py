...
def one_hot_max(x, axis=-1):
    ...
    return tf.one_hot(tf.argmax(x, axis=axis), depth=x.get_shape()[-1], dtype=x.dtype)
    ...
    def loss(self, predictions, targets):
        ...
        if self.soft_targets == "normalized":
            ...
            loss = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=(1.0 - smoothing) * targets + smoothing * 0.5, logits=predictions, name="ensemble_loss"
            )
            loss = tf.reduce_mean(loss, axis=-1)
        else:
            loss = tf.nn.softmax_cross_entropy_with_logits(labels=targets, logits=predictions, name="ensemble_loss")
        return loss
...
class PlaceCellEnsemble(CellEnsemble):
    ...
    def __init__(self, n_cells, stdev=0.35, pos_min=-5, pos_max=5, seed=None, soft_targets=None, soft_init=None):
...
class HeadDirectionCellEnsemble(CellEnsemble):
    ...
    def __init__(self, n_cells, concentration=20, seed=None, soft_targets=None, soft_init=None):
        super(HeadDirectionCellEnsemble, self).__init__(n_cells, soft_targets, soft_init)
