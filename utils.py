...
def get_place_cell_ensembles(env_size, neurons_seed, targets_type, lstm_init_type, n_pc, pc_scale):
...
def get_head_direction_ensembles(neurons_seed, targets_type, lstm_init_type, n_hdc, hdc_concentration):
...
def encode_initial_conditions(init_pos, init_hd, place_cell_ensembles, head_direction_ensembles):
    ...
    for ens in place_cell_ensembles:
        initial_conds.append(tf.squeeze(ens.get_init(init_pos[:, tf.newaxis, :]), axis=1))
    for ens in head_direction_ensembles:
        initial_conds.append(tf.squeeze(ens.get_init(init_hd[:, tf.newaxis, :]), axis=1))
...
def get_scores_and_plot(
    ...
    s = [scorer.calculate_ratemap(xy[:, 0], xy[:, 1], act[:, i]) for i in list(range(n_units))]
    # Get the scores
    ...
    np.savez_compressed(f"./checkpoints/plot{epoch}.npy", s, sac)
    for i in list(range(n_units)):
...
def plot_trajectory(real_postion, predict_postion, epoch):
    np.savez_compressed(f"./checkpoints/trajectory{epoch}.npy", real_postion, predict_postion)
    rows = 3
    cols = 3
    ...
    for i in range(rows * cols):
        ...
        plt.plot(real_postion[i, :, 0], real_postion[i, :, 1], "b", label="Truth Path")
        # plot predicted trajectory
        plt.plot(predict_postion[i, :, 0], predict_postion[i, :, 1], "go", label="Predicted Path")
        plt.legend()
    fig.savefig(f"./photo/predictedTrajectory{epoch}.png")
