...
def displaced_linear_initializer(input_size, displace, dtype=tf.float32):
    stddev = 1.0 / numpy.sqrt(input_size)
...
class GridCellsRNNCell(snt.RNNCore):
    ...
    def _build(self, inputs, prev_state):
        ...
        if self.training and self._dropoutrates_bottleneck is not None:
            ...
            dropped_pops = [
                tf.nn.dropout(pop, rate, name="dropout") for rate, pop in zip(self._dropoutrates_bottleneck, scale_pops)
            ]
        ...
        ens_outputs = [
            snt.Linear(
                ...
                regularizers={"w": tf.contrib.layers.l2_regularizer(self._bottleneck_weight_decay)},
                initializers={
                    "w": displaced_linear_initializer(self._nh_bottleneck, self._init_weight_disp, dtype=tf.float32)
                },
                name="pc_logits",
            )(bottleneck)
    ...
    def output_size(self):
        ...
        return tuple([ens.n_cells for ens in self._target_ensembles] + [self._nh_bottleneck, self._nh_lstm])
...
class GridCellsRNN(snt.AbstractModule):
    ...
    def get_all_variables(self):
        return super(GridCellsRNN, self).get_variables() + self._core.get_variables()
